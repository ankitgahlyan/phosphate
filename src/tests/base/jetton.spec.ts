...
describe.each([
    ...
    beforeAll(async () => {
        ...
        jettonMinter = blockchain.openContract(
            await MinterWrapper.fromInit(0n, deployer.address, defaultContent, true),
        )
    ...
    it("not a minter admin should not be able to mint jettons", async () => {
        ...
        expect(unAuthMintResult.transactions).toHaveTransaction({
            ...
            to: jettonMinter.address,
            aborted: true,
            exitCode: 700, // JettonMinterSharded.errors["Incorrect sender"],
        })
        expect(await deployerJettonWallet.getJettonBalance()).toEqual(initialJettonBalance)
    ...
    it("not wallet owner should not be able to send jettons", async () => {
        ...
        expect(sendResult.transactions).toHaveTransaction({
            ...
            to: deployerJettonWallet.address,
            aborted: true,
            exitCode: 700, // JettonWallet.errors["Incorrect sender"],
        })
        expect(await deployerJettonWallet.getJettonBalance()).toEqual(initialJettonBalance)
    ...
    it("impossible to send too much jettons", async () => {
        ...
        expect(sendResult.transactions).toHaveTransaction({
            ...
            to: deployerJettonWallet.address,
            aborted: true,
            exitCode: 709, // JettonWallet.errors["Incorrect balance after send"],
        })
        expect(await deployerJettonWallet.getJettonBalance()).toEqual(initialJettonBalance)
        expect(await notDeployerJettonWallet.getJettonBalance()).toEqual(initialJettonBalance2)
    })
    ...
    it("correctly sends forward_payload in place", async () => {
        ...
        */
        expect(sendResult.transactions).toHaveTransaction({
            ...
            value: forwardAmount,
            body: beginCell()
                .storeUint(JettonMinterSharded.opcodes.JettonNotification, 32)
    ...
    it("check revert on not enough tons for forward", async () => {
        ...
        expect(sendResult.transactions).toHaveTransaction({
            ...
            aborted: true,
            exitCode: 703, // JettonWallet.errors["Insufficient amount of TON attached"],
        })
    ...
    describe("Bounces", () => {
        ...
        it("minter should restore supply on internal_transfer bounce", async () => {
            ...
            expect(outMsgSc.preloadUint(32)).toEqual(JettonMinterSharded.opcodes.JettonTransferInternal)
        ...
        it("wallet should restore balance on burn_notification bounce", async () => {
            ...
            const outMsgSc = firstOutMsg.body.beginParse()
            expect(outMsgSc.preloadUint(32)).toEqual(JettonMinterSharded.opcodes.JettonBurnNotification)
    ...
    it("minter should only accept burn messages from jetton wallets", async () => {
        ...
        const burnNotification = (amount: bigint, addr: Address) => {
            return beginCell()
                .storeUint(JettonMinterSharded.opcodes.JettonBurnNotification, 32)
                .storeUint(0, 64)
                .storeCoins(amount)
                .storeAddress(addr)
                .storeAddress(deployer.address)
        ...
        expect(res.transactions).toHaveTransaction({
            ...
            to: jettonMinter.address,
            aborted: true,
            exitCode: 724, // JettonMinterSharded.errors["Unauthorized burn"],
        })
    ...
    it("report correct discovery address", async () => {
        ...
        */
        ...
        expect(discoveryResult.transactions).toHaveTransaction({
            ...
            to: deployer.address,
            body: beginCell()
                .storeUint(JettonMinterSharded.opcodes.TakeWalletAddress, 32)
                .storeUint(0, 64)
    ...
    it.skip("Minimal discovery fee", async () => {
        ...
        expect(discoveryResult.transactions).toHaveTransaction({
            ...
            to: jettonMinter.address,
            aborted: true,
            exitCode: JettonMinterSharded.errors["Out of gas error"],
        })
        /*
    ...
    */
    ...
    it("can not send to masterchain", async () => {
        ...
        expect(sendResult.transactions).toHaveTransaction({
            ...
            to: deployerJettonWallet.address,
            aborted: true,
            exitCode: 734, // JettonWallet.errors["Not a basechain address"],
        })
    })
